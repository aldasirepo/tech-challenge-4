...
def update_rule(flag_name):
    ...
    # Safe SQL using psycopg2.sql
    set_clause = sql.SQL(", ").join(sql.SQL(f) for f in fields)
    query = sql.SQL(
        "UPDATE targeting_rules SET {set_clause} "
        "WHERE flag_name = %s RETURNING *"
    ).format(set_clause=set_clause)  # nosec B608
